...
def autocorrect_En(statement):
    s = requests.Session()
    r = s.get("http://192.168.10.253:5000/autocorrect_En/", params={'statement': statement})
...
def translate_video_Ar(file_name):
    s = requests.Session()
    r = s.get("http://192.168.10.253:5000/translate_video_Ar/", params={'file_name': file_name})
    return r.json()['prediction']
